...
def run(opts):
    shopping_api = Shopping(debug=opts.debug, appid=opts.appid, config_file=opts.yaml,
                   warnings=True)
    #print("Shopping samples for SDK version %s" % ebaysdk.get_version())

    try:
        finding_api = finding(debug=opts.debug, appid=opts.appid, config_file=opts.yaml, warnings=True)

        api_request = {
            'keywords': u'perkasie PA',
            'itemFilter': [
                {'name': 'Condition',
                 'value': 'Used'},
                {'name': 'LocatedIn',
        ...
        f_response = shopping_api.execute('findItemsAdvanced', api_request)
        
        print(finding_api.response_dom)
        
        nodes = finding_api.response_dom().xpath('//ItemID')

        itemIds = [n.text for n in nodes]
        print(itemIds)
        
        item_id_found=f_response.item.itemId
        print(item_id_found)
        #print(response.dict)
        s_response = shopping_api.execute('GetSingleItem', item_id_found)
        #print(response.dict())
        #print(response)
        #dump(api)
        #for property, value in vars(response.reply.ItemArray).iteritems():
        #    print property, ": ", value
        print("--------------")    
        print("Matching Titles:")
        for item in s_response.reply.ItemArray.Item:
